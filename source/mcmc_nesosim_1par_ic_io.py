...
#TODO: make easier to switch between single & multiple params?
...
# parameter to be optimized: 'wpf' for wind packing, 
# 'llf' for blowing snow factor

#OPT_PAR_NAME = 'wpf'
OPT_PAR_NAME = 'llf'

# density climatology specifications
if USE_DENS:
	DENS_STR = '_density'
elif USE_DENS_CLIM:
...
par_list = [par_vals] # now an n*m list (m = # of pars)
loglike_list = [p0]
...
# scale to appropriate value if applicable (don't do this for ICF)
...
for i in range(ITER_MAX):
	...
	if i%1000 == 0 and i > 0:
		...
		fname = 'mcmc_output_i{}_u_{}_p0_{}_s0_{}_{}noseed.h5'.format(i,UNCERT,PARS_INIT[0],PAR_SIGMA[0],DENS_STR)
...
fname = 'mcmc_output_i{}_u_{}_p0_{}_s0_{}_{}noseed.h5'.format(ITER_MAX,UNCERT,PARS_INIT[0],PAR_SIGMA[0],DENS_STR)
print(ITER_MAX)
print(fname)
write_to_file(fname, stats_list, par_list, loglike_list, par_names, rejected_stats, rejected_pars, rejected_lls)
