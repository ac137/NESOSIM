...
def get_OIB_and_mask(dx, yearT, depthBudget, date_start):#, days_ds, diff_ds):
	...
	# grab density data also
	densData = depthBudget['density']
	densData = np.ma.masked_where(iceConc<0.15, densData)
...
#		maskDay[np.where(snowDepthM>0.8)]=1
		...
		densMM = densM[maskDay<1] # density units kg/m^3
		...
		densMMall.extend(densMM) # keep in kg/m^3

	return snowOIBMall, snowDepthMMall, densMMall
...
# density values from stations
dens_vals = np.arange(12) # placeholder
dens_uncert = dens_vals # placeholder

# mean density march-april from station
# multiply by 1000 to convert from g/cm^3 to kg/m^3
station_dens_m_a_mean = 0.3321641360089186*1000
station_dens_m_a_uncert = 0.03*1000 # uncertainty in density
...
def main(params, uncert):
	...
	for year1 in range(startYear, endYear):
		...
		snowDepthOIByr, snowDepthMMyr, densMMyr = get_OIB_and_mask(dx, year2, budgets,date_start)
		snowDepthOIBAll.extend(snowDepthOIByr)
		snowDepthMMAll.extend(snowDepthMMyr)
		densMMAll.extend(densMMyr)
	...
	densMMAll = np.array(densMMAll)
	# snow density arrays from station data; DS for 'drifting station'
	densDSAll = np.ones(np.shape(densMMAll))*station_dens_m_a_mean
	densUncert = station_dens_m_a_uncert
